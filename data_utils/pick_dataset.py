...
class PICKDataset(Dataset):
    ...
    def __init__(self, files_name: str = None,
        ...
        files_paths = [self.boxes_and_transcripts_folder.joinpath(Path(f).stem + '.csv') for f in self.files_list['file_name'].to_list()]
        if self.training:
            self.lengths = [self.read_gt_file_transcript_length(f) for f in files_paths]
        else:
            self.lengths = [self.read_ocr_file_transcript_length(f) for f in files_paths]
    ...
    def read_gt_file_transcript_length(self, filepath: str):
        ...
        return len(''.join(x for x in res))


    def read_ocr_file_transcript_length(filepath: str):
        with open(filepath, 'r', encoding='utf-8') as f:
            document_text = f.read()
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            boxes_and_transcripts_file = self.boxes_and_transcripts_folder.joinpath(
                Path(dataitem['file_name']).stem + '.csv')
            ...
            boxes_and_transcripts_file = self.boxes_and_transcripts_folder.joinpath(
                Path(self.files_list[index]).stem + '.csv')
...
class BatchCollateFn(object):
    ...
    def __init__(self, training: bool = True):
        """
        # AAG: this normalization for dataset from article.
        ...
        """
        # AAG: this mean and std value have been obtained from train dataset statistics.
        self.trsfm = transforms.Compose([transforms.ToTensor(),
                                         transforms.Normalize(mean=[0.9539, 0.9538, 0.9539],
                                         std=[0.1803, 0.1806, 0.1808])
                                        ])
    ...
    def __call__(self, batch_list: List[Document]):
        ...
        # whole image, B, C, H, W
        # AAG: images pixels values must be in range 0-1 before normalization.
        image_batch_tensor = torch.stack([self.trsfm(x.whole_image/255) for x in batch_list], dim=0).float() 
