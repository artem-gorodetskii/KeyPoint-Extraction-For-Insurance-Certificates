...
MAX_BOXES_NUM = 405  # limit max number boxes of every documents
MAX_TRANSCRIPT_LEN = 25  # limit max length text of every box
...
class Document:
    def __init__(self, boxes_and_transcripts_file: Path, image_file: Path,
        ...
        try:
            ...
            for x in boxes_and_transcripts_data:
                self.segments_order.extend([x[0]]*len(x[-1])) # added
