...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.scale = torch.sqrt(torch.FloatTensor([char_embedding_dim])) #
        ...
        elif image_encoder == 'se_resnet18':
            self.cnn = resnet.se_resnet18(output_channels=image_feature_dim)
        elif image_encoder == 'se_resnet34':
            self.cnn = resnet.se_resnet34(output_channels=image_feature_dim)
        elif image_encoder == 'se_resnet50':
            self.cnn = resnet.se_resnet50(output_channels=image_feature_dim)
        elif image_encoder == 'se_resnet101':
            self.cnn = resnet.se_resnet101(output_channels=image_feature_dim)
        elif image_encoder == 'se_resnet152':
            self.cnn = resnet.se_resnet152(output_channels=image_feature_dim)

        else:
            raise NotImplementedError()

        # AAG: Instead of Conv2d layer we will apply two Linear projections
        #self.conv = nn.Conv2d(image_feature_dim, out_dim, self.roi_pooling_size)
        #self.bn = nn.BatchNorm2d(out_dim)

        # AAG: Two Linear projections and dropout for encoding image segments to fixed-length vector
        self.flat_dropout = nn.Dropout(self.dropout)
        self.flat_projection1 = nn.Linear(image_feature_dim*roi_pooling_size[0]*roi_pooling_size[1], image_feature_dim*4)
        self.flat_projection2 = nn.Linear(image_feature_dim*4, out_dim*1)

        # AAG: Instead of summation we will apply concatenation and projection to text and image segments embedding 
    ...
    def forward(self, images, boxes_coordinate, transcripts,
        ...
        # AAG: Encoding of image segments to fixed-length vectors using Dense layers
        image_segments = torch.flatten(image_segments, 1)
        image_segments = self.flat_dropout(image_segments)
        image_segments = self.flat_projection1(image_segments)
        image_segments = self.flat_projection2(image_segments)
        ...
        # AAG: training is better without scaling factor.
        ...
        """
        # AAG: we first concatenate transcripts_segments and image_segments embedding and then project them using Linear layer.
        out = torch.cat([transcripts_segments, image_segments], dim = -1)
        ...
        # AAG: removed. This dropout decrease model perfomance.
